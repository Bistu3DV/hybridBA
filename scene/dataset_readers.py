...
from utils.graphics_utils import getWorld2View2, focal2fov, fov2focal, camera_intrinsic_transform
import numpy as np
import json
...
# class CameraInfo:
#     def __init__(self, uid, R, T, FovX, FovY, image, image_path,
#                  image_name, width, height, intrinsic, extrinsic_c2w,
#                  image_tensor, img_feats=None):
#         uid: int
#         R: np.array
#         T: np.array
#         FovY: np.array
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        intrinsic_matrix = camera_intrinsic_transform(FovX, FovY, width, height)
        extrinsic_w2c = np.zeros((4,4),dtype=float)
        extrinsic_w2c[-1, -1] = 1.0
        extrinsic_w2c[:3, :3] = R
        extrinsic_w2c[:3, 3] = T
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              ...
                              height=height, intrinsic=intrinsic_matrix, extrinsic_c2w=extrinsic_c2w,
                              image_tensor=image_tensor, img_feats=[None,None,None,None])
...
def readColmapSceneInfo(path, images, eval, lod, llffhold=8):
    ...
    if eval:
        if lod > 0:
            ...
            # 每3张图片取一张
            selected_cam_infos = cam_infos[::3]
            train_cam_infos = [c for idx, c in enumerate(selected_cam_infos) if idx % 4 != 0]
            test_cam_infos = [c for idx, c in enumerate(selected_cam_infos) if idx % 4 == 0]
